...
ATOM_FEATURES = {
    ...
    # ],
    'atom_in_member_ring': [3, 4, 5, 6, 7, 8]
}
...
# -1 for ATOM_FEATURES['atom_in_member_ring'] is multi-hot vector, instead of one-hot vector
# +1 for checking whether the atom is in ring
ATOM_FDIM = sum(
    len(choices) + 1 for choices in ATOM_FEATURES.values()) + 2 - 1 + 1
EXTRA_ATOM_FDIM = 0
BOND_FDIM = 14 - 4 + 6
# -4 for close the bond type feautrization, +6 for appending whether bond is in N-number(3~8) ring size
EXTRA_BOND_FDIM = 0
REACTION_MODE = None
EXPLICIT_H = False
REACTION = False
...
def multi_encoding_member_ring(atom, choices: List[int]) -> List[int]:
    """
    Create a multi-hot vector specifically for the atom feature how many member ring it belongs to.
    """
    encoding = [0]*len(choices)
    for i, member in enumerate(choices):
        if atom.IsInRingSize(member):
            encoding[i] = 1
    return encoding


def multi_encoding_member_ring_bond(bond, choices: List[int]) -> List[int]:
    """
    Create a multi-hot vector specifically for the atom feature how many member ring it belongs to.
    """
    encoding = [0]*len(choices)
    for i, member in enumerate(choices):
        if bond.IsInRingSize(member):
            encoding[i] = 1
            return encoding
    return encoding
...
def atom_features(atom: Chem.rdchem.Atom, functional_groups: List[int] = None) -> List[Union[bool, int, float]]:
    ...
    if atom is None:
        ...
        features = onek_encoding_unk(atom.GetAtomicNum() - 1, ATOM_FEATURES['atomic_num']) + \
            ...
            multi_encoding_member_ring(atom, ATOM_FEATURES['atom_in_member_ring']) + \
    ...
    # onek_encoding_unk(int(atom.GetHybridization()), ATOM_FEATURES['hybridization']) + \ # chemprop default feature
...
def bond_features(bond: Chem.rdchem.Bond) -> List[Union[bool, int, float]]:
    ...
    if bond is None:
        ...
        fbond += multi_encoding_member_ring_bond(bond, [3, 4, 5, 6, 7, 8])
    return fbond
...
class MolGraph:
    ...
    * :code:`overwrite_default_bond_features`: A boolean to overwrite default bond descriptors.
    ...
    def __init__(self, mol: Union[str, Chem.Mol, Tuple[Chem.Mol, Chem.Mol]],
        ...
        if type(mol) == str:
            if self.is_reaction:
                mol = (make_mol(mol.split(">")[0], self.is_explicit_h), make_mol(
                    mol.split(">")[-1], self.is_explicit_h))
            else:
        ...
        if not self.is_reaction:
            ...
            if self.reaction_mode == 'reac_prod':
                self.f_atoms = [x+y[MAX_ATOMIC_NUM+1:]
...
def mol2morgan(smiles_batch: List[str], hidden_size: int = 2048) -> torch.FloatTensor:
    """
    Converts a list of SMILES strings to a batch of Morgan fingerprints.

    :param smiles_batch: A list of SMILES strings.
    :param args: Arguments.
    :return: A FloatTensor containing the Morgan fingerprints for the molecules
    """
    Morgan_batch = []
    for i, smiles in enumerate(smiles_batch):
        mol = Chem.MolFromSmiles(smiles)
        fp = AllChem.GetMorganFingerprintAsBitVect(
            mol, 2, nBits=hidden_size)
        Morgan_batch.append(fp)
    Morgan_batch = torch.Tensor(Morgan_batch)
    return Morgan_batch
