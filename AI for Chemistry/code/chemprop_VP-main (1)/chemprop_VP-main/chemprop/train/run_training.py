...
from chemprop.data import get_class_sizes, get_data, MoleculeDataLoader, MoleculeDataset, set_cache_graph, split_data, divmaxScaler
...
def pred_true_curve(args: TrainArgs, pred_vals, true_vals, model_idx: int, cur_name: str, unit_name: str):
    ...
    plt.xlabel('Target ' + unit_name)
    plt.xlim(math.floor(mini), math.ceil(maxi))
    plt.ylim(math.floor(mini), math.ceil(maxi))
    plt.savefig(f'{args.save_dir}_{model_idx}_{cur_name}.png',
...
def run_training(args: TrainArgs,
    ...
    if args.manual_temp_scaling is not None:
        train_ts = train_data.temperatures()
        val_ts = val_data.temperatures()
        test_ts = test_data.temperatures()
        T_scaler = divmaxScaler().fit(train_ts)
        T_scaler.maxs = args.manual_temp_scaling
        scaled_ts = T_scaler.transform(train_ts).tolist()
        train_data.set_ts(scaled_ts)
        scaled_ts = T_scaler.transform(val_ts).tolist()
        val_data.set_ts(scaled_ts)
    ...
    for model_idx in range(args.ensemble_size):
        ...
        save_checkpoint(os.path.join(save_dir, MODEL_FILE_NAME), model, scaler,
                        features_scaler, atom_descriptor_scaler, bond_feature_scaler, T_scaler, args)
        ...
        for epoch in trange(args.epochs):
            ...
            if args.minimize_score and avg_val_score < best_score or \
                ...
                save_checkpoint(os.path.join(save_dir, MODEL_FILE_NAME), model, scaler, features_scaler,
                                atom_descriptor_scaler, bond_feature_scaler, T_scaler, args)
            else:
                cnt += 1
