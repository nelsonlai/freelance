...
def train(model: MoleculeModel,
    ...
    for batch in tqdm(data_loader, total=len(data_loader), leave=False):
        ...
        T_batch = torch.Tensor(batch.temperatures()
                               ).unsqueeze(-1).to(args.device)

        mask = torch.tensor([[x is not None for x in tb]
                            for tb in target_batch], dtype=torch.bool)
        ...
        if args.dataset_type == 'multiclass':
            ...
            loss = torch.cat([loss_func(preds[:, target_index, :], targets[:, target_index]).unsqueeze(
                1) for target_index in range(preds.size(1))], dim=1) * target_weights * data_weights * mask
