...
from chemprop.data import StandardScaler, MoleculeDataset, preprocess_smiles_columns, get_task_names, divmaxScaler
...
def save_checkpoint(path: str,
    ...
    state = {
        ...
        'T_scaler': {
            'maxs': T_scaler.maxs
        } if T_scaler is not None else None
    }
...
def load_frzn_model(model: torch.nn,
    ...
    if loaded_args.number_of_molecules == 1 & current_args.number_of_molecules == 1:
        encoder_param_names = ['encoder.encoder.0.W_i.weight', 'encoder.encoder.0.W_h.weight',
        ...
        if current_args.frzn_ffn_layers > 0:
            ffn_param_names = [['ffn.'+str(i*3+1)+'.weight', 'ffn.'+str(i*3+1)+'.bias']
        ...
        if (current_args.checkpoint_frzn is not None) & (not current_args.freeze_first_only) & (not (current_args.frzn_ffn_layers > 0)):
            loaded_encoder_param_names = ['encoder.encoder.0.W_i.weight', 'encoder.encoder.0.W_h.weight',
                                          'encoder.encoder.0.W_o.weight', 'encoder.encoder.0.W_o.bias']*current_args.number_of_molecules
            model_encoder_param_names = [['encoder.encoder.'+str(mol_num)+'.W_i.weight', 'encoder.encoder.'+str(mol_num)+'.W_h.weight', 'encoder.encoder.'+str(
                mol_num)+'.W_o.weight', 'encoder.encoder.'+str(mol_num)+'.W_o.bias'] for mol_num in range(current_args.number_of_molecules)]
        ...
        if (loaded_args.number_of_molecules) != (current_args.number_of_molecules):
            raise Exception('Number of molecules in checkpoint_frzn ({}) must match current model ({}) OR equal to 1.'.format(
                loaded_args.number_of_molecules, current_args.number_of_molecules))

        if current_args.freeze_first_only:
            raise Exception('Number of molecules in checkpoint_frzn ({}) must be equal to 1 for freeze_first_only to be used.'.format(
                loaded_args.number_of_molecules))

        if (current_args.checkpoint_frzn is not None) & (not (current_args.frzn_ffn_layers > 0)):
            encoder_param_names = [['encoder.encoder.'+str(mol_num)+'.W_i.weight', 'encoder.encoder.'+str(mol_num)+'.W_h.weight', 'encoder.encoder.'+str(
                mol_num)+'.W_o.weight', 'encoder.encoder.'+str(mol_num)+'.W_o.bias'] for mol_num in range(current_args.number_of_molecules)]
            encoder_param_names = [
...
def load_scalers(path: str) -> Tuple[StandardScaler, StandardScaler, StandardScaler, StandardScaler, divmaxScaler]:
    """
    ...
    if 'T_scaler' in state.keys():
        T_scaler = divmaxScaler(
            state['T_scaler']['maxs'], replace_nan_token=0) if state['T_scaler'] is not None else None
    else:
        T_scaler = None

    return scaler, features_scaler, atom_descriptor_scaler, bond_feature_scaler, T_scaler
